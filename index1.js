// ============================================================
// index1.js — ТЕСТОВАЯ ВЕРСИЯ (загрузка из data.json)
// Клон index.js, но данные берутся из локального файла
// ============================================================
...
async function loadAllData() {
    try {
        ...
        if (!response.ok) throw new Error('Ошибка загрузки данных из data.json');
	...
	console.log('🏆 Параметры турнира загружены (из data.json):', tournamentParams);
	    ...
	    console.log('🏆 Данные о сборных загружены (из data.json):', Object.keys(teamsData).length, 'стран');
        }
        ...
        console.error('Ошибка загрузки данных из data.json:', err);

        document.getElementById('table-wrapper').innerHTML = `
            <div class="loading-overlay" style="color:#a00;">
                ❌ Ошибка загрузки data.json<br>
                <span style="font-size:0.8rem;color:#888;">Для загрузки данных файл data.json должен лежать в той же папке, что и index.html</span>
            </div>
        `;
        return false;
    }
...
async function sendScoreToSheet(matchIndex, score) {
    // === ТЕСТОВАЯ ВЕРСИЯ: ЗАПИСЬ ОТКЛЮЧЕНА ===
    console.log('⚠️ ТЕСТОВАЯ ВЕРСИЯ: Запись счёта отключена (data.json)');
    alert('⚠️ В тестовой версии (index1.js) запись счёта отключена. Используйте index.html для реальной работы.');
    return;
